...
AFDM_trt_2019$shrimpDay <- ifelse(AFDM_trt_2019$poolid == '1b',   QPB1_day,
...
#use ifelse statements to put shrimp count into corresponding correct rows
AFDM_trt_2019$shrimpNight <- ifelse(AFDM_trt_2019$poolid == '1b',   QPB1_night,
...
sink("leafdecomp_shrimpefx.jags")    ## give it a name
cat("
    model { 
### specify how the data relate to variables; this is the ecological process 
    for(i in 1:nobs.2019){ #  total number afdm remaining observations!
        ...
        AFDM.2019[i] ~ dnorm(mu[i], tau)  #each observation is a drawn from a distribution with mean AFDM remaining for that day, treatment and pool
         

        #version below gets omits the stream effect
        log(mu[i])<- log(100) +  (b0*(1-treatment.2019[i]) + b1*treatment.2019[i] + b2*shrimp_day[i]*(1- treatment.2019[i]) + 
        random.effect[pool.2019[i]])*day.2019[i] 
        
       #  k[i]<-(b0*(1-treatment.2019[i]) + b1*treatment.2019[i] + b2*shrimp_day[i]*(1- treatment.2019[i]) + 
        #random.effect[pool.2019[i]])
    }
...
## let's derive the estimated treatment decay, each year and stream
  ...
  #  c.decay.2019A <- b0
   # ec.decay.2019A <- b1
    # c.decay.2019B <- b0 + b2
    # ec.decay.2019B <- b1 + b2
    
    predicted.k.control <- b0+b2*shrimp.for.plots
   # predicted.k.exclusion <- b1 
   
   shrimp.effect.control <- (1+b2)/(1+ b0)

### define the priors for the unknowns in the model
### we now have for unknown regression coefficients: fixed-effect influences of EC, 2018, 2019 (i.e., year, compared to 2017),
### and PB (compared to PA)

  #this says we have no idea what the decay rate is; it is centered on 0, but precision = 1/variance = 0.001.
 ### can change to an informative prior later
  b0 ~ dnorm(0, 0.001)
 b1 ~ dnorm(0, 0.001) #this says we have no idea how the treatment affects decay rate
 b2 ~ dnorm(0, 0.001) # or how PB differs from PA in control
...
win.data.2019 <- list(AFDM.2019 = AFDM.2019, npools.2019=npools.2019, nobs.2019=nobs.2019, day.2019=day.2019, treatment.2019=treatment.2019, pool.2019=pool.2019,
                      PB.2019=PB.2019, shrimp_day=shrimp_day, shrimp.for.plots=shrimp.for.plots)

## set starting points for the unknowns to be estimated in the regression
inits.2019 <- function(){list(b0 = 0, b1 = 0, b2=0,  sigma = 10, sigma.pool = 1)}

## specify the parameters for which we want posterior estimates - mean decay rate and treatment effect, and variances
params.2019<-c("b0","b1", "b2" ,"sigma","sigma.pool", "predicted.k.control", "predicted.k.exclusion", "shrimp.effect.control") 
...
#version with stream ommitted
out.decay.2019.1 <- jags(win.data.2019, inits.2019, params.2019, "leafdecomp_shrimpefx.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, working.directory = getwd())
print(out.decay.2019.1, dig = 4)

#make table of output
Model2_table <- xtable(out.decay.2019.1$BUGSoutput$summary)
#option 1 export right to csv where you can make stylist changes and rename rows
write.csv(Model2_table, "C:/Users/Max/Desktop/UGA/Pringle Lab/R/msDecomp/decomp_rate/R_out/model.4.table.csv")#if using on different computer will need to update this for location to work
...
figure.pub<- ggplot(data=alltogether, aes(x=shrimp.for.plots.OR, y=-meanvalues.1, fill=Treatment)) +
  geom_ribbon(aes(ymin=-lowerci.1, ymax=-upperci.1), alpha=0.6, show.legend = F)+
  geom_point(aes(x=shrimp.for.plots.OR, y=-meanvalues.1, shape=Treatment), size=0.5,  fill="black" )+
  theme_classic(base_size = global_size) +
  xlab("Mean shrimp abundance" ) +
  ylab(expression(~Predicted ~decomposition ~rate ~(italic(-k) ~day^-1)))+
  scale_fill_grey(start=0.0, end=0.5)
ggsave("shrimp.decomp.tiff", figure.pub, width=8.6,height=7.5, dpi=400, units="cm" )
